...
import { coaching, jobs, analytics, type CoachingSession, type IRIData, type Job } from '@/lib/api'
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, PieChart, Pie, Cell, RadarChart, Radar, PolarGrid, PolarAngleAxis } from 'recharts'
import { Mic, Plus, ChevronRight, TrendingUp } from 'lucide-react'
...
type CoachTab = 'overview' | 'analytics'
...
export default function CoachingPage() {
  ...
  async function loadAnalytics() {
    ...
    try {
      const res = await analytics.graduate()
  ...
  const COLORS = ['var(--yellow)', 'var(--blue-core)', 'var(--green)', 'var(--red)', 'var(--cyan-bright)']
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid var(--border-subtle)' }}>
        {([
          ...
          <button key={t.key}
            onClick={() => { setCoachTab(t.key); if (t.key === 'analytics') loadAnalytics() }}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px 16px', fontSize: 13, fontWeight: 500, color: coachTab === t.key ? 'var(--text-primary)' : 'var(--text-muted)', borderBottom: `2px solid ${coachTab === t.key ? 'var(--blue-core)' : 'transparent'}`, marginBottom: -1, transition: 'all 0.15s' }}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          {coachTab === 'analytics' && (
            analyticsLoading || !analyticsData ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {/* IRI progress line chart */}
                <div className="card">
                  <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600, marginBottom: 20 }}>IRI Score Progress</h3>
                  {analyticsData.iri_history.length < 2 ? (
                    <EmptyState title="Not enough data" description="Complete at least 2 sessions to see your progress chart." />
                  ) : (
                    <ResponsiveContainer width="100%" height={220}>
                      <LineChart data={analyticsData.iri_history}>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                        ...
                        <Line type="monotone" dataKey="score" stroke="var(--blue-core)" strokeWidth={2.5} dot={{ fill: 'var(--blue-bright)', r: 4, strokeWidth: 0 }} name="Overall" />
                        <Line type="monotone" dataKey="communication" stroke="var(--green)" strokeWidth={1.5} dot={false} name="Communication" />
                        <Line type="monotone" dataKey="technical" stroke="var(--cyan-bright)" strokeWidth={1.5} dot={false} name="Technical" />
                        <Line type="monotone" dataKey="confidence" stroke="var(--yellow)" strokeWidth={1.5} dot={false} name="Confidence" />
                      </LineChart>
                    </ResponsiveContainer>
                  )}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  {/* Radar chart */}
                  <div className="card">
                    <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600, marginBottom: 20 }}>Skill Breakdown (Latest)</h3>
                    {analyticsData.total_sessions === 0 ? (
                      <EmptyState title="No sessions yet" description="Complete a session to see your breakdown." />
                    ) : (
                      <ResponsiveContainer width="100%" height={220}>
                        <RadarChart data={analyticsData.radar_data}>
                          <PolarGrid stroke="rgba(255,255,255,0.1)" />
                          <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} />
                          <Radar name="Score" dataKey="score" stroke="var(--blue-core)" fill="var(--blue-core)" fillOpacity={0.2} />
                          <Tooltip contentStyle={tooltipStyle} />
                        </RadarChart>
                      </ResponsiveContainer>
                    )}
                  ...
                  <div className="card">
                    <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600, marginBottom: 20 }}>Application Status</h3>
                    {analyticsData.application_breakdown.length === 0 ? (
                      <EmptyState title="No applications yet" description="Apply to jobs to see your breakdown." />
                    ) : (
                      <ResponsiveContainer width="100%" height={220}>
                        <PieChart>
                          <Pie data={analyticsData.application_breakdown} dataKey="value" nameKey="status"
                            cx="50%" cy="50%" outerRadius={80}
                            label={({ status, percent }: any) => `${status} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                            {analyticsData.application_breakdown.map((_: any, i: number) => (
                ...
                {/* Applications trend */}
                <div className="card">
                  <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600, marginBottom: 20 }}>Applications Submitted — Last 30 Days</h3>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  {[
                    { label: 'Total Sessions',    value: analyticsData.total_sessions, color: 'var(--blue-bright)' },
                    { label: 'Current IRI Score', value: analyticsData.current_iri,    color: 'var(--green)' },
                    { label: 'Profile Complete',  value: `${analyticsData.profile_completeness}%`, color: 'var(--cyan-bright)' },
                  ].map(s => (
                    <div key={s.label} className="card" style={{ textAlign: 'center' }}>
                      <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 28, fontWeight: 500, color: s.color, lineHeight: 1 }}>{s.value}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8 }}>{s.label}</div>
                    </div>
                  ))}
                </div>
              </div>
