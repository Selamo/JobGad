...
export default function JobsPage() {
  ...
  async function loadBrowse() {
    ...
    try {
      ...
      if (Array.isArray(res)) {
        ...
        setBrowseJobs((res as any).jobs ?? (res as any).listings ?? [])
  ...
  async function handleRunMatches() {
    ...
    try {
      ...
      toast(e.message || 'Matching failed. Make sure your profile has skills added.', 'error')
    } finally {
      setRunning(false) }
  }
  ...
  async function handleExplain(jobId: string) {
    try {
      ...
      toast(e.message || 'Could not load explanation', 'error')
    }
  }
  ...
  const scoreColor = (s: number) =>
    s >= 0.8 ? 'var(--green)' : s >= 0.6 ? 'var(--blue-bright)' : 'var(--yellow)'
  ...
  return (
    <AppShell
      title="Jobs"
      subtitle="AI matches and job listings"
      actions={
        tab === 'matches' ? (
    ...
    >
        ...
        {(['matches', 'browse'] as Tab[]).map(t => (
          <button key={t} onClick={() => { setTab(t); setPage(1) }}
            style={{
              ...
              borderBottom: `2px solid ${tab === t ? 'var(--blue-core)' : 'transparent'}`,
              marginBottom: -1, transition: 'all 0.15s',
            }}>
            {t === 'matches'
              ? `AI Matches${matches.length > 0 ? ` (${matches.length})` : ''}`
              : `Browse All Jobs${total > 0 ? ` (${total})` : ''}`
            }
          </button>
        ))}
      </div>
      ...
      {tab === 'browse' && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
            ...
            <input className="input" placeholder="Search jobs..."
              value={searchQ} onChange={e => { setSearchQ(e.target.value); setPage(1) }}
              style={{ paddingLeft: 36 }} />
          </div>
          <select className="input" style={{ width: 150 }} value={filterType}
      ...
      {loading ? (
        ...
        matches.length === 0 ? (
          <div className="card" style={{ padding: 48, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, maxWidth: 340, margin: '0 auto 16px' }}>
              Click Run AI matching to find jobs that match your profile. Make sure you have added skills to your profile first.
            </p>
            <button className="btn btn-primary" onClick={handleRunMatches} disabled={running}>
              {running ? <Spinner size="sm" /> : <Zap size={14} />}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {matches.map(match => (
              <div key={match.id} className="card card-interactive"
                style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                {/* Score */}
                <div style={{
                  width: 52, height: 52, borderRadius: 10, flexShrink: 0,
                  background: `${scoreColor(match.similarity_score)}18`,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 4 }}>
                    <div>
                      <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600, marginBottom: 2 }}>
                        {match.job?.title}
                      </h3>
                  ...
                  <div style={{ display: 'flex', gap: 14, marginBottom: 8 }}>
                    ...
                    {match.job?.salary_range && (
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{match.job.salary_range}</span>
                    )}
                  </div>

                  {match.match_reason && (
                    <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.55, marginBottom: 12, fontStyle: 'italic' }}>
                      {match.match_reason}
                    </p>
                  )}

                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button className="btn btn-primary btn-sm" onClick={() => openApply(match.job)}>
                      Apply now
                    </button>
                    <button className="btn btn-ghost btn-sm" onClick={() => handleExplain(match.job.id)}>
                      Explain match <ChevronRight size={12} />
                    </button>
                    {match.status === 'suggested' && (
                      <button className="btn btn-ghost btn-sm" onClick={() => handleUpdateStatus(match.id, 'saved')}>
                        Save
                      </button>
                    )}
                    {match.status !== 'rejected' && (
                      <button className="btn btn-ghost btn-sm" onClick={() => handleUpdateStatus(match.id, 'rejected')}
        ...
        browseJobs.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {browseJobs.map(job => (
                <div key={job.id} className="card card-interactive"
                  style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 6 }}>
                      <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 15, fontWeight: 600 }}>
                        {job.title}
                      </h3>
                    ...
                    {job.salary_range && (
                      <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>{job.salary_range}</p>
                    )}
      ...
      <Modal
        open={showApplyModal}
        onClose={() => { setShowApplyModal(false); setCoverLetter('') }}
            ...
            <>
              <div style={{ background: 'var(--bg-elevated)', borderRadius: 8, padding: '12px 14px', marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  {applyTarget.company} · {applyTarget.location}
                </div>
              </div>
              ...
              {explanation.skill_overlap?.matched?.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <p className="label-caps" style={{ marginBottom: 8 }}>
                    Skills you have ({explanation.skill_overlap.matched.length})
                  </p>
